...
#### В процессе сделано:
 - Было протестировано создание тестовых машин с помощью **Molecule**
 - Были протестированы роли с помощью **Testinfra**
 - Был модифицирован тест к роли **db** - **db/molecule/default/tests/test_default.py** для проверки того, что БД слушает на порту 27017
 - Были использованы роли **app** и **db** в playbooks **packer_app.yml** и **packer_db.yml** для работы шаблонов packer-а
 - Для настройки **nginx** была дополнена конфигурация в **Vagrantfile**

#### Как запустить проект:
 - Для запуска инфраструктуры с помощью **Vagrant** использовать команду **vagrant up**
 - Для запуска провижинера использовать команды **vagrant provision dbserver** и **vagrant provision appserver**
 - Для удаления созданных машин с помощью **Vagrant** использовать команду **vagrant destroy -f**
 - Для создания заготовки тестов для роли **db** использовать команду **molecule init scenario --scenario-name default -r db -d vagrant** в в директории с ролью **ansible/roles/db**
 - Для создания тестовых машин с помощю **Molecule** использовать команду **molecule create**
 - Для прогона тестов с помощью **Molecule** использовать команду **molecule verify**

#### Как проверить работоспособность:
 - После запуска инфраструктуры с помощью **Vagrant** приложение должно быть доступно по ссылке **http://10.10.10.20**
 
#### PR checklist
 - [x] Выставил label **Homework-12** с номером домашнего задания
 - [x] Выставил label **Ansible** с номером домашнего задания
